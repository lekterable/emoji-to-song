		...
		spotifyAuthorize(
			...
			(err, spotifyRes, body) => {
				...
				spotifyGetInfo(
					request,
					'artists',
					artists.map((artist) => artist.spotify_id).join(),
					body.access_token,
					(err, spotifyRes, body) => {
						...
						return res.status(200).json({
							success: true,
							message: mergeResults(artists, body.artists)
						})
					}
				)
			}
				...
				(err, spotifyRes, body) => {
					...
					Artist.findOne({ spotify_id: artist.spotify_id }, (err, doc) => {
						...
						if (!doc) {
							...
							return res
								.status(201)
								.json({ success: true, message: 'Artist updated' })
						}
						doc.emojis = removeDuplicates(split(doc.emojis + artist.emojis, ''))
						doc.save()
						return res
							.status(201)
							.json({ success: true, message: 'Artist saved' })
					})
				}
			)
		}
